...
class MaintenanceListView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.user.groups.filter(name='admin').exists() or self.request.user.groups.filter(name='manager').exists():
            ...
            context['maintenance_type'] = set(qs_filter.filter(service_company__user=self.request.user).values_list('maintenance_type__name', flat=True))
            context['car'] = set(qs_filter.filter(service_company__user=self.request.user).values_list('car__vin', flat=True))
            ...
            context['maintenance_type'] = set(qs_filter.filter(car__user=self.request.user).values_list('maintenance_type__name', flat=True))
            context['car'] = set(qs_filter.filter(car__user=self.request.user).values_list('car__vin', flat=True))
            context['service_company'] = set(qs_filter.filter(car__user=self.request.user).values_list('service_company__name', flat=True))
...
class ComplaintListView(PermissionRequiredMixin, ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.user.groups.filter(name='admin').exists() or self.request.user.groups.filter(name='manager').exists():
            ...
            context['failure_node'] = set(qs_filter.filter(car__user=self.request.user).values_list('failure_node', flat=True))
            context['recovery_method'] = set(qs_filter.filter(car__user=self.request.user).values_list('recovery_method__name', flat=True))
            context['service_company'] = set(qs_filter.values_list('service_company__name', flat=True))
